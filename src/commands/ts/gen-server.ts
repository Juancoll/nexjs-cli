...
class CommandOptions extends Options {
    ...
    @option({
        flag: 'n',
        required: true,
        description: 'package base name -> [namespace].api.server will be created.',
    })
...
export default class extends CommandBase {
    ...
    async execute(
        ...
        const target = join(resolve(context.cwd, options.output), `${options.namespace}.api.server`);
        ...
        const iconFolder = join(target, 'public', 'images');
        await this.create_icon(view.style.icon, iconFolder, 16);
        await this.create_icon(view.style.icon, iconFolder, 32);
        await this.create_icon(view.style.icon, iconFolder, 256);
        await this.create_icon(view.style.icon, iconFolder, 512);
        //#endregion
