...
class CommandOptions extends Options {
    ...
    @option({
        flag: 'n',
        required: false,
        description: 'wsclient package name',
    })
    public name: string;
...
export default class extends CommandBase {
    ...
    async execute(
        ...
        //#region [2] ROOT FOLDER
        console.log('[2] ROOT FOLDER');
        const rootSource = assets.getPath('templates/root');
        const rootView = { name: config.name }
        FS.copyFolder(rootSource, target, (s, t, c) => {
            console.log(`  |- [create]  ${t}`);
            return mustache.render(c, rootView);
        });
        //#endregion   
        ...
        const routesSource = assets.getPath('templates/routes/wsRoutes.ts');
        const routesTarget = join(target, 'src', 'router', 'routes-partial', `wsRoutes.ts`);
