...
export class WSHRestEventConverter extends CodeConverterBase<MethodDeclaration, WSRestMethod> {
    ...
    public convert(input: MethodDeclaration): WSRestMethod {
        let output: WSRestMethod = new WSRestMethod({
            name: input.getName(),
    ...
    //#region [ public ]
    public isRest(method: MethodDeclaration) {
    ...
    private extractDecorators(method: MethodDeclaration): RDecorator[] {
        ...
        var decorators = method.getDecorators().filter(x => x.getName() != 'Rest');
    ...
    private extractParams(method: MethodDeclaration): RParam[] {
        ...
        method.getParameters().forEach(param => {
            var decorator = param.getDecorators().find(x => x.getName() == 'Data')
            if (decorator) {
                var paramDecorator = new RDecorator({ name: decorator.getName() })
        ...
        var rootDataParam = result.find(x => !x.decorators[0].options);
        if (rootDataParam) {
