...
export class HubNotification<TCredentials, TData> {
    //#region [ fields ]
    ...
    constructor(hub: HubClient, service: string, event: string) {
        ...
        hub.onReceive.sub(async publication => {
            if (publication.service == service && publication.event == event) {
    ...
    on(action: (value: TData) => void | Promise<void>): HubNotification<TCredentials, TData> {
        this._actions.push(action);
        return this;
    }
    off(): HubNotification<TCredentials, TData> {
        this._actions = [];
        return this;
    }
